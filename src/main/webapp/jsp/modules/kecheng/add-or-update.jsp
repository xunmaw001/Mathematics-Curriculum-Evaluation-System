                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="laoshiId" name="laoshiId" type="hidden">
                                    <div class="form-group col-md-6 kechengUuidNumberDiv">
                                        <label>课程编号</label>
                                    ...
                                    <div class="form-group col-md-6 kechengVideoDiv">
                                        <label>视频</label>
                                        <video id="kechengVideoV" style="margin-top: 10px" src="" width="100" height="100" controls="controls"></video>
                                        <input name="file" type="file" id="kechengVideoupload"
                                               class="form-control-file">
                                        <input name="kechengVideo" id="kechengVideo-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6 kechengFileDiv">
                                        <label>课程文件</label>
                                        <input name="file" type="file" class="form-control-file" id="kechengFileupload">
                                        <label id="kechengFileName"></label>
                                        <input name="kechengFile" id="kechengFile-input" type="hidden">
                                    </div>
                                    ...
                                    <div class="form-group col-md-6 kechengPingfenDiv">
                                        <label>课程评分</label>
                                        <input style="width: 450px" id="kechengPingfen" name="kechengPingfen" class="form-control"
                                               onchange="kechengPingfenChickValue(this)" placeholder="课程评分">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#kechengVideoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('kechengVideoV').setAttribute('src',photoUrl);
                document.getElementById('kechengVideo-input').setAttribute('value',photoUrl);
            }
        });


        $('#kechengFileupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                document.getElementById('kechengFile-input').setAttribute('value', baseUrl + 'file/download?fileName=' + data.result.file);
                document.getElementById('kechengFileName').innerHTML = "上传成功!";
            }
        });
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function kechengPingfenChickValue(e){
        ...
        function kechengDeleteChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[1-9][0-9]*$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                laoshiId: "required",
                kechengUuidNumber: "required",
                kechengName: "required",
                kechengTypes: "required",
                kechengVideo: "required",
                kechengFile: "required",
                kechengContent: "required",
                kechengPingfen: "required",
            },
            messages: {
                laoshiId: "老师不能为空",
                kechengUuidNumber: "课程编号不能为空",
                kechengName: "课程名称不能为空",
                kechengTypes: "课程类型不能为空",
                kechengVideo: "课程视频不能为空",
                kechengFile: "课程文件不能为空",
                kechengContent: "课程详情不能为空",
                kechengPingfen: "课程评分不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addkecheng != null && addkecheng != "" && addkecheng != "null") {
            //注册表单验证
            $(validform());
            $("#kechengUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
    ...
    function showVideo() {
    <!--  当前表的视频  -->
        $("#kechengVideoV").attr("src",ruleForm.kechengVideo);
