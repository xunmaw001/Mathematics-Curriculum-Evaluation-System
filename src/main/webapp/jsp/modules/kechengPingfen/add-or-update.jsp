    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑课程评价</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">课程评价管理</li>
                        <li class="breadcrumb-item active">编辑课程评价</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">课程评价信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 kecheng">
                                        <label>课程编号</label>
                                        <input style="width: 450px" id="kechengUuidNumber" name="kechengUuidNumber" class="form-control"
                                               placeholder="课程编号" readonly>
                                    </div>
                                    <div class="form-group col-md-6 kecheng">
                                        <label>课程名称</label>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="kechengId" name="kechengId" type="hidden">
                                    <div class="form-group col-md-6 kechengPingfenPingfenDiv">
                                        <label>评分(不能大于10)</label>
                                        <input style="width: 450px" id="kechengPingfenPingfen" name="kechengPingfenPingfen" class="form-control"
                                               onchange="kechengPingfenPingfenChickValue(this)" placeholder="评分(不能大于10)">
                                    </div>
                                    <div class="form-group  col-md-6 kechengPingfenContentDiv">
                                        <label>评价内容</label>
                                        <input id="kechengPingfenContentupload" name="file" type="file">
                                        <script id="kechengPingfenContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var kechengPingfenContentUe = UE.getEditor('kechengPingfenContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="kechengPingfenContent" id="kechengPingfenContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#kechengPingfenContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('kechengPingfenContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            if(window.sessionStorage.getItem('role') != '课程'){//当前登录用户不为这个
                if($("#kechengId") !=null){
            ...
            httpJson("kechengPingfen/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addkechengPingfen');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.kechengPingfenContent != null && ruleForm.kechengPingfenContent != 'null' && ruleForm.kechengPingfenContent != '' && $("#kechengPingfenContentupload").length>0) {

            var kechengPingfenContentUeditor = UE.getEditor('kechengPingfenContentEditor');
            kechengPingfenContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.kechengPingfenContent != null){
                    mes = ''+ ruleForm.kechengPingfenContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                kechengPingfenContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#kechengPingfenContentupload").length>0) {
            var kechengPingfenContentEditor = UE.getEditor('kechengPingfenContentEditor');
            if (kechengPingfenContentEditor.hasContents()) {
                $('#kechengPingfenContent-input').attr('value', kechengPingfenContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function kechengPingfenPingfenChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                xueshengId: "required",
                kechengId: "required",
                kechengPingfenPingfen: "required",
                kechengPingfenContent: "required",
            },
            messages: {
                xueshengId: "学生不能为空",
                kechengId: "课程不能为空",
                kechengPingfenPingfen: "评分不能为空",
                kechengPingfenContent: "评价内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addkechengPingfen = window.sessionStorage.getItem("addkechengPingfen");
        if (addkechengPingfen != null && addkechengPingfen != "" && addkechengPingfen != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("kechengPingfen/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#kechengPingfenPingfen").val(ruleForm.kechengPingfenPingfen);
        $("#kechengPingfenContent").val(ruleForm.kechengPingfenContent);
